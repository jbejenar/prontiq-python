...
export const BILLING_EVENT_VERSION = 2;
...
export const billingUsageEventV2Schema = z.object({
  version: z.literal(BILLING_EVENT_VERSION),
  eventId: z.string().regex(/^bevt_[a-f0-9]{32}$/),
  occurredAt: z.string().datetime(),
  orgId: z.string().regex(/^org_[A-Za-z0-9]+$/),
  apiKeyHash: z.string().min(32),
  keyPrefix: z.string().min(1),
  product: z.string().min(1),
  billingEndpointKey: z.string().min(1),
...
export type BillingUsageEventV2 = z.infer<typeof billingUsageEventV2Schema>;
...
export function deriveBillingUsageEventId(input: BillingEventIdInput): string {
  const identity = input.orgId ?? input.customerId;
  if (!identity) {
    throw new Error("orgId is required for billing event id derivation");
  }
  const raw = [
    "v2",
    identity,
...
export function deriveLegacyBillingUsageEventId(input: BillingEventIdInput & { customerId: string }): string {
  const raw = [
    "v1",
    input.customerId,
    input.apiKeyHash,
...
export function deriveLagoExternalSubscriptionIdForOrg(orgId: string): string {
  if (!/^org_[A-Za-z0-9]+$/.test(orgId)) {
    throw new Error("orgId must be a Clerk organization id");
  }
