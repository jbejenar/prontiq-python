...
export async function lookupPostcode(postcode: string) {
  const response = await getOpenSearchClient().search({
    ...
    body: {
      ...
      aggs: {
        localities: {
          terms: { field: "localityName", size: 50 },
          aggs: {
...
export async function lookupSuburb(suburb: string, state?: string) {
  const must: AnyRecord[] = [{ term: { localityName: suburb.toUpperCase() } }];
  if (state) {
    must.push({ term: { state: state.toUpperCase() } });
  }
  ...
  return {
    ...
    address_count: getTotalValue(response.body.hits.total),
  };
}
