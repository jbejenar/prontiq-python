...
import { ProntiqDefaultError } from "../models/errors/prontiq-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ProntiqDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ProntiqDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ProntiqDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ProntiqDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
