...
- **Clerk webhook handler** (`POST /webhooks/clerk`) wired to the existing `PqApi` — verifies Svix signature, branches on `organizationMembership.created` + `role === "admin"`, calls `createProvisioningService().provisionOrg(...)` to write the ORG envelope + audit row + best-effort welcome email. New `PqClerkWebhook` Lambda + 3 GitHub Environment secrets (`CLERK_WEBHOOK_SECRET`, `STRIPE_SECRET_KEY`, `CLERK_SECRET_KEY`) sourced via the deploy workflows' `env:` block (matches existing `WELCOME_EMAIL_FROM` pattern) + CloudWatch error alarm wired to `PqIngestAlerts` + fail-fast deploy guard if any required secret is unset on dev/prod. Operator runbook in `docs/runbooks/clerk-webhook.md`. Closes the functional half of P1B.05; recovery endpoint `POST /v1/account/setup` follows in PR 3 of P1B.05.
