...
modify the contents of the `src/prontiq/lib/` and `examples/` directories.
...
```sh
$ pip install git+ssh://git@github.com/prontiq/prontiq-platform.git
```
...
You can release to package managers by using [the `Publish PyPI` GitHub action](https://www.github.com/prontiq/prontiq-platform/actions/workflows/publish-pypi.yml). This requires a setup organization or repository secret to be set up.
