...
**Stack (planned — P1B/P1C):** Clerk (auth), Stripe (billing). API keys are DynamoDB-native (SHA-256 hash-based; see ADR-001, ARCHITECTURE.MD §5.5). Webhook handler source exists in `packages/webhooks/` but isn't deployed yet; legacy `unkey.ts` pending deletion in a follow-up code-cleanup PR.
