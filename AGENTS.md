...
**Stack (planned — remaining P1B / all P1C):** SES bounce handler (P1B.08), month-close (P1B.11), `/account` dashboard (P1C). `POST /webhooks/stripe` (P1B.06) is implemented, and the hourly billing cron (P1B.10) now pushes family-level usage into Stripe meters from DynamoDB usage counters using replay-safe pending meter markers on the current hash's usage row. The architecture source of truth now treats **credits** as the customer-facing billing unit and endpoint weights as the internal rating model.
