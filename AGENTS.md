...
**Commercial architecture posture:** P1B.21 completed the Lago migration go-live cleanup. Lago is the commercial system of record, Stripe is the payment rail only through Lago, Prontiq keeps hot-path credit enforcement in DynamoDB, and SQS buffers billing events away from the request path. `customerId` is the platform-owned customer identity (`pq_cust_<ulid>`); Clerk org IDs, Stripe customer IDs, and Lago `lago_id` values are linkage fields only. The P1B.15 SQS producer is feature-flagged by `BILLING_EVENTS_ENABLED`; the P1B.16 worker forwards queued events to Lago with `transaction_id = eventId`, `external_subscription_id = pq_sub_<customer ulid>`, and `properties.credits = creditDelta`. P1B.17 adds `POST /webhooks/lago` for async subscription/invoice reconciliation back into local enforcement state, keyed by `X-Lago-Unique-Key` and gated by `LAGO_WEBHOOK_RECONCILIATION_ENABLED`. P1B.21 disabled the retained prod smoke API key after final accepted event `bevt_f7833d581725b732d04d3eed3fd7c484`; do not reuse or reactivate that fixture. Keep Lago and Stripe off the API hot path.
