...
**Commercial architecture posture:** the **currently shipped** billing path is still Stripe-centric (`POST /webhooks/stripe`, hourly billing cron, month-close, and Stripe-backed provisioning). The **target v-next** commercial architecture is Lago-centered: Lago becomes the commercial system of record, Stripe becomes the payment rail only, Prontiq keeps hot-path credit enforcement in DynamoDB, and SQS buffers billing events away from the request path. `customerId` is the platform-owned customer identity (`pq_cust_<ulid>`); Clerk org IDs, Stripe customer IDs, and Lago `lago_id` values are linkage fields only. The P1B.15 SQS producer is feature-flagged by `BILLING_EVENTS_ENABLED`; the P1B.16 worker forwards queued events to Lago with `transaction_id = eventId`, `external_subscription_id = pq_sub_<customer ulid>`, and `properties.credits = creditDelta`. P1B.17 adds `POST /webhooks/lago` for async subscription/invoice reconciliation back into local enforcement state, keyed by `X-Lago-Unique-Key` and gated by `LAGO_WEBHOOK_RECONCILIATION_ENABLED`. P1B.18a certification must use `pnpm --filter @prontiq/control-plane lago:smoke:event` or an equivalent production-contract helper for smoke billing events; do not hand-build Lago transaction IDs. P1B.18b is the production go-live cleanup gate: clean, disable, relabel, or explicitly retain prod smoke artifacts before customer-facing billing depends on Lago. Keep Lago and Stripe off the API hot path.
