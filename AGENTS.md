...
**Stack (live):** SST v4 + Pulumi, Hono + @hono/zod-openapi on Lambda (ARM64), OpenSearch, DynamoDB. SDKs auto-generated by Speakeasy from committed OpenAPI spec. **Two ingress paths to org provisioning** are live in prod, both calling `createProvisioningService().provisionOrg(...)` from `@prontiq/control-plane`: (1) Clerk webhook (`POST /webhooks/clerk`) verifies Svix signature, gates on org admin role; (2) `POST /v1/account/setup` (Clerk-JWT-authenticated) is the dashboard recovery path when the webhook missed delivery. Same idempotency invariant; any combination of webhook + recovery calls collapses to one envelope, one Stripe customer, and one audit row. API keys are DynamoDB-native (SHA-256 hash-based; see ADR-001, ARCHITECTURE.MD §5.5).

**Commercial architecture posture:** the **currently shipped** billing path is still Stripe-centric (`POST /webhooks/stripe`, hourly billing cron, month-close, and Stripe-backed provisioning). The **target v-next** commercial architecture is Lago-centered: Lago becomes the commercial system of record, Stripe becomes the payment rail only, Prontiq keeps hot-path credit enforcement in DynamoDB, and SQS buffers billing events away from the request path. `customerId` is the platform-owned customer identity (`pq_cust_<ulid>`); Clerk org IDs, Stripe customer IDs, and Lago `lago_id` values are linkage fields only. The P1B.15 SQS producer is feature-flagged by `BILLING_EVENTS_ENABLED`; the P1B.16 worker forwards queued events to Lago with `transaction_id = eventId`, `external_subscription_id = pq_sub_<customer ulid>`, and `properties.credits = creditDelta`. P1B.17 adds `POST /webhooks/lago` for async subscription/invoice reconciliation back into local enforcement state, keyed by `X-Lago-Unique-Key` and gated by `LAGO_WEBHOOK_RECONCILIATION_ENABLED`. P1B.18a certification must use `pnpm --filter @prontiq/control-plane lago:smoke:event` or an equivalent production-contract helper for smoke billing events; do not hand-build Lago transaction IDs. Retained prod smoke fixtures should remain available for remaining Lago migration tickets but must be labelled/inventoried as test-only; final destructive cleanup is P1B.21 after P1B.20. Keep Lago and Stripe off the API hot path.

**P1B.18a status:** do not mark it complete yet. The 2026-04-26 closeout audit found dev/prod API-produced Lago usage smoke accepted and smoke fixtures inventoried, but webhook certification is still open: `LAGO_WEBHOOK_RECONCILIATION_ENABLED=false` is deployed in both webhook Lambdas and no completed Lago webhook-ledger rows exist. Finish dev/prod HMAC webhook smoke before treating P1B.18 as fully unblocked.
...
packages/
  ...
  webhooks/       -> @prontiq/webhooks        Clerk webhook handler + legacy/current Stripe billing webhook during Lago migration
