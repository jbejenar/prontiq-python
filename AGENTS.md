...
**Stack (live):** SST v4 + Pulumi, Hono + @hono/zod-openapi on Lambda (ARM64), OpenSearch, DynamoDB. SDKs auto-generated by Speakeasy from committed OpenAPI spec. **Clerk webhook handler** (`POST /webhooks/clerk`) live in prod — verifies Svix signature, gates on org admin role, calls `createProvisioningService().provisionOrg(...)` from `@prontiq/control-plane` to write the ORG envelope (Stripe customer + DDB row + audit row + best-effort welcome email). API keys are DynamoDB-native (SHA-256 hash-based; see ADR-001, ARCHITECTURE.MD §5.5).

**Stack (planned — remaining P1B / all P1C):** `POST /v1/account/setup` recovery endpoint (P1B.05 PR 3/3), Stripe subscription webhook (P1B.06), SES bounce handler (P1B.08), billing cron (P1B.10), month-close (P1B.11), `/account` dashboard (P1C). The `packages/webhooks/src/stripe.ts` file is still a stub awaiting P1B.06.
...
packages/
  ...
  control-plane/  -> @prontiq/control-plane   provisionOrg service + writeAudit helpers (consumed by webhooks; will be consumed by api in P1B.05 PR 3/3)
  api/            -> @prontiq/api             Hono API on Lambda (address routes today; /v1/account/setup pending in P1B.05 PR 3/3)
...
- `control-plane` -> `webhooks` (and `api` in P1B.05 PR 3/3)
