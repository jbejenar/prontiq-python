...
from ._exceptions import ProntiqError, APIResponseValidationError
...
class BaseAPIResponse(Generic[R]):
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        if (
            ...
            raise TypeError("Pydantic models must subclass our base model type, e.g. `from prontiq import BaseModel`")
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `prontiq._streaming` for reference",
        )


class StreamAlreadyConsumed(ProntiqError):
    """
