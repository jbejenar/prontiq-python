...
- **P1B.18a complete (2026-04-26).** The Lago runtime is deployed in dev and
  ...
  GitHub Environments and deployed Lambdas, and alert policy is ALARM-only for
  email-backed operational alarms. `COUNTER_PERIOD_SOURCE` remained `calendar`
  until the P1B.19 cutover posture. Dev/prod webhook certification completed
  with unique keys
...
- **P1C.01a (2026-04-27)**: Landing visual reskin shipped. New token vocabulary (`--info`, `--warn`, `--muted-2`, `--border-strong`, `--surface-hover`, `--accent-glow`, `--scanline`) plus `--prontiq-widget-*` namespaced widget tokens emitted from `@prontiq/tokens`. `@prontiq/web-component` consumes the namespaced widget tokens with safe fallbacks for third-party embedders. `apps/landing` ships the terminal topbar, KPI row with static sparklines, endpoints table, slash-prefixed metric cards, health panel, foot strip, and tri-state System/Light/Dark theme picker. Instrument Serif + JetBrains Mono now self-hosted via `next/font/google` (previously the page rendered in browser-default Times+Courier). Shared content schema gains a required `releases.gnaf` ISO-format source-of-truth field with `superRefine` cross-validation that fails on any drift between `releases.gnaf`, the hero `g-naf` metaItem, and KPI deltas referencing g-naf — eliminates the malformed-date class of bug at parse time. Demo proxy + unlock middleware untouched.
...
- `docs/decisions/031-stripe-legacy-runtime-retirement.md`
- `docs/decisions/032-lago-period-cutover.md`
