...
- **P1B.05 PR 2/3**: Clerk webhook handler (`POST /webhooks/clerk` on the existing `PqApi`). Verifies Svix signature, branches on `organizationMembership.created` + `role === "admin"`, calls `createProvisioningService().provisionOrg(...)` from `@prontiq/control-plane`. New `PqClerkWebhook` Lambda + 3 SST secrets (`ClerkWebhookSecret`, `StripeSecretKey`, `ClerkSecretKey`) + CloudWatch error alarm wired to `PqIngestAlerts`. Operator runbook in `docs/runbooks/clerk-webhook.md`. Recovery endpoint `POST /v1/account/setup` deferred to PR 3 of P1B.05.
